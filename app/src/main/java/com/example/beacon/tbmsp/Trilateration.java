package com.example.beacon.tbmsp;
...
public class Trilateration{
    public static double[] Compute(Point p1,Point p2,Point p3){
        ...
        if(k>p1.gr()*2){
            i = null;
        }else if(i[0]<-90||i[0]>90||i[1]<-180||i[1]>180){
            i=null;
        }
        return i;
    }
    ...
    private static double[] C(double a,double b,double c,double d){
        return new double[]{
                a*Math.cos((Math.PI/180)*d)-b*Math.sin((Math.PI/180)*d),a*Math.sin((Math.PI/180)*d)+b*Math.cos((Math.PI/180)*d),c
        };
    }
    ...
    private static double E(double a,double b,double c,double d){
        double e=Math.PI,f=e*a/180,g=e*c/180,h=b-d,i=e*h/180,j=Math.sin(f)*Math.sin(g)+Math.cos(f)*Math.cos(g)*Math.cos(i);if(j>1){j=1;}j=Math.acos(j);j=j*180/e;j=j*60*1.1515;j=j*1.609344;
        return j;
    }
}